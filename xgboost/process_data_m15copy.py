...
def check_partern_2hl_less(df, current_index, main_entry_direction):
    x_zigzag_etf = json.loads(df.loc[current_index, "output_ta"])["zigzag_data"]["x_zigzag_data"]
    y_zigzag_etf = json.loads(df.loc[current_index, "output_ta"])["zigzag_data"]["y_zigzag_data"]
    type_zigzag_etf = json.loads(df.loc[current_index, "output_ta"])["zigzag_data"]["type_zigzag_data"]
    list_intersec_etf = json.loads(df.loc[current_index, "output_ta"])["list_intersec"]
    
    re_notrade = json.loads(df.loc[current_index, "output_ta"])["key_level"]["resistance2"]
    sp_notrade = json.loads(df.loc[current_index, "output_ta"])["key_level"]["support1"]
    ...
    if main_entry_direction == "BUY":
        ...
        if number_intersec != None and number_intersec != len(x_zigzag_etf):
            min_element = 100000
            # index_min = y_zigzag_htf_htf.index(min(y_zigzag_htf_htf[number_intersec:]))
            for i in range(number_intersec, len(x_zigzag_etf)):
                ...
                # else:
                elif index_L1 != None and y_zigzag_etf[i] < min_element and y_zigzag_etf.index(y_zigzag_etf[i]) - index_L1 == 2:
                    min_element = y_zigzag_etf[i]
                    index_L1 = y_zigzag_etf.index(min_element)
                elif index_L1 != None and y_zigzag_etf[i] < min_element and y_zigzag_etf.index(y_zigzag_etf[i]) - index_L1 > 2:
        ...
        if (index_L1 != None
                ...
                if (abs(price_L2 - price_L1) < (price_H1 - price_L1)/3
                    and index_L2 < len(y_zigzag_etf) - 1
                    and len(x_zigzag_etf) - index_L2 > 1

                    and time_to_candle(x_zigzag_etf[index_L1], x_zigzag_etf[index_H1], time_frame="m15") > 5 # Đáy 1 đỉnh 1 cách nhau 5 cây nến
                    and abs(price_H1 - price_L1) > 5 # Đáy 1 đỉnh 1 cách nhau 5 giá
                    and time_to_candle(x_zigzag_etf[index_L1], x_zigzag_etf[index_H1], time_frame="m15") > 5 # Đáy 2 đỉnh 1 cách nhau 5 cây nến
                    and (abs(price_H1 - price_L1) > abs(price_L1-price_H1)*0.7) # giá chị tương quan đáy phù hợp
                    and abs(price_H2 - price_L2) > 5 # Đáy 2 đỉnh 1 cách nhau 5 giá
                    and (abs(price_H2 - price_L2) > abs(price_L2-price_H1)*0.7)  if (price_L2 > price_L1) else (abs(price_H2 - price_L1) > abs(price_L2-price_H1)*0.7)# giá chị tương quan đáy phù hợp
                    and abs(price_L1 - max(y_zigzag_etf[index_H1:])) < 20 # giá chạy quá 15 giá thì thôi k vào
                    ):
                    if index_max_2 > index_L2:
                        entry_price = [min(y_zigzag_etf[index_L1 + 1:]) - 1, min(y_zigzag_etf[index_L1 + 1:]) + 2]
                    else:
                        entry_price = [y_zigzag_etf[index_L1] - 1, y_zigzag_etf[index_L1] + 2]
    ...
    if main_entry_direction == "SELL":
        ...
        if number_intersec != None and number_intersec != len(x_zigzag_etf):
            ...
            for i in range(number_intersec, len(x_zigzag_etf)):
                ...
                # else:
                elif index_H1 != None and y_zigzag_etf[i] > max_element and y_zigzag_etf.index(y_zigzag_etf[i]) - index_H1 == 2:
                    max_element = y_zigzag_etf[i]
                    index_H1 = y_zigzag_etf.index(max_element)
                elif index_H1 != None and y_zigzag_etf[i] > max_element and y_zigzag_etf.index(y_zigzag_etf[i]) - index_H1 > 2:
        ...
        if (index_H1 != None
                ...
                if ( 
                    abs(price_H1 - price_H2) < (price_H1 - price_L1)/3
                    and index_H1 < len(y_zigzag_etf) - 1
                    and len(x_zigzag_etf) - index_H1 > 1
                    and time_to_candle(x_zigzag_etf[index_H1], x_zigzag_etf[index_L1], time_frame="m15") > 5
                    and abs(price_H1 - price_L1) > 5
                    and (abs(price_H2 - price_L1) > abs(price_L1-price_H1)*0.7)
                    and abs(price_H2 - price_L2) > 5
                    and (abs(price_H2 - price_L2) > abs(price_L1-price_H2)*0.7) if (price_H2<price_H1) else (abs(price_H1 - price_L2) > abs(price_L1-price_H2)*0.7)
                    and abs(price_H1 - min(y_zigzag_etf[index_H1:])) < 20
                    # abs(max(y_zigzag_etf[index_H1 + 1:])- y_zigzag_etf[index_H1]) < (y_zigzag_etf[index_H1] - min(y_zigzag_etf[index_H1:self.last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))]))/2
                    # and self.last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:])) < len(y_zigzag_etf) - 1
                    # and len(x_zigzag_etf) - self.last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:])) > 1
                    # # and time_to_candle(x_zz_htf[index_max], x_zz_htf[y_zz.index(max(y_zz[index_max + 1:]))], time_frame="m15") > 15
                    # and time_to_candle(x_zigzag_etf[y_zigzag_etf.index(min(y_zigzag_etf[index_H1:self.last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))]))], x_zigzag_etf[self.last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))], time_frame="m15") > 3
                    # and abs(y_zigzag_etf[y_zigzag_etf.index(min(y_zigzag_etf[index_H1:y_zigzag_etf.index(max(y_zigzag_etf[index_H1 + 1:]))]))] - y_zigzag_etf[self.last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))]) > 5
                    # and time_to_candle(x_zigzag_etf[self.last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))], x_zigzag_etf[-1], time_frame="m15") > 6
                    # and abs(y_zigzag_etf[self.last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))] - y_zigzag_etf[y_zigzag_etf.index(min(y_zigzag_etf[self.last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:])) :]))]) > 5
                        
                    ):
                        if index_min_2 > index_H2:
                            entry_price = [max(y_zigzag_etf[index_H1 + 1:]) + 1, max(y_zigzag_etf[index_H1 + 1:]) - 2]
                        else:
                            entry_price = [y_zigzag_etf[index_H1] + 1, y_zigzag_etf[index_H1] - 2]
...
def round_down_to_hour(self, time_str):
...
def check_partern_1hl_gearing_zone(df, current_index, main_entry_direction):
    x_zigzag_etf = json.loads(df.loc[current_index, "output_ta"])["zigzag_data"]["x_zigzag_data"]
    y_zigzag_etf = json.loads(df.loc[current_index, "output_ta"])["zigzag_data"]["y_zigzag_data"]
    type_zigzag_etf = json.loads(df.loc[current_index, "output_ta"])["zigzag_data"]["type_zigzag_data"]
    list_intersec_etf = json.loads(df.loc[current_index, "output_ta"])["list_intersec"]
    ...
    if main_entry_direction == "BUY":
        ...
        if number_intersec != None and number_intersec != len(x_zigzag_etf):
            ...
            for i in range(number_intersec, len(x_zigzag_etf)):
                if y_zigzag_etf[i] < min_element:
                    min_element = y_zigzag_etf[i]
                    index_min = y_zigzag_etf.index(min_element)
                else:
                    if y_zigzag_etf[i] < min_element and y_zigzag_etf.index(y_zigzag_etf[i]) - index_min == 2:
                        min_element = y_zigzag_etf[i]
                        index_min = y_zigzag_etf.index(min_element)
                    if y_zigzag_etf[i] < min_element and y_zigzag_etf.index(y_zigzag_etf[i]) - index_min > 2:
                        if min_element - y_zigzag_etf[i] > 5:
                            ...
                            index_min = y_zigzag_etf.index(min_element)
        if (index_min != None
                and abs(current_price - y_zigzag_etf[index_min]) < 10
                ):
            time_index_previus = round_down_to_hour(x_zigzag_etf[index_min-1])
            time_current = str(df.loc[current_index, "Date"])
            
            list_htf, list_date = get_unique_htf_candles(time_index_previus, time_current)
            list_tight_range = []
            list_boxes = []
            for j, data_point in enumerate(list_htf):
                list_boxes = caculate_tight_range(j, list_htf)
            ...
            if len(list_boxes) > 0:
                ...
                for n in range(index_box, len(list_htf)):
                    if list_htf[n][1] > max_box + 3 and current_price > min_box:
                        ...
                        current_signal = {
                            ...
                            "entry_validation_info": {
                                ...
                                "hl_1": [[], [y_zigzag_etf[-2]], "high", [], [str(x_zigzag_etf[-2])]],
                                "hl_2": [[], [y_zigzag_etf[-2]], "high", [], [str(x_zigzag_etf[-2])]],
                                "signal_date_time": str(df.loc[current_index, "Date"]),
                                "trading_action_child": trading_action,
                            },
                        }
                        pattern_type = 2
        ...
        if number_intersec != None and number_intersec != len(x_zigzag_etf):
            ...
            for i in range(number_intersec, len(x_zigzag_etf)):
                if index_max == None:
                    if y_zigzag_etf[i] > max_element:
                        max_element = y_zigzag_etf[i]
                        index_max = y_zigzag_etf.index(max_element)
                else:
                    if y_zigzag_etf[i] > max_element and y_zigzag_etf.index(y_zigzag_etf[i]) - index_max == 2:
                        max_element = y_zigzag_etf[i]
                        index_max = y_zigzag_etf.index(max_element)
                    if y_zigzag_etf[i] > max_element and y_zigzag_etf.index(y_zigzag_etf[i]) - index_max > 2:
                        if y_zigzag_etf[i] - max_element > 5:
                            ...
                            index_max = y_zigzag_etf.index(max_element)
        if (index_max != None
                and current_price - y_zigzag_etf[index_max] < 10
                ):
            time_index_previus = round_down_to_hour(x_zigzag_etf[index_max-1])
            time_current = str(df.loc[current_index, "Date"])
            ...
            if len(list_boxes) > 0:
                ...
                index_box = (max(box[0][0], box[1][0]))
                for n in range(index_box, len(list_htf)):
                    if list_htf[n][1] < min_box- 3 and current_price < max_box:
...
def get_unique_htf_candles(self, start_time, end_time):
    ...
    while start_idx <= end_idx:
        ...
        if "htf" not in self.data[mid] or "Date" not in self.data[mid]["htf"]:
            # Skip invalid entries by moving forward
            start_idx = mid + 1
            continue
            
        time_mid = self.data[mid]["htf"]["Date"]
    ...
    for i in range(start_search_idx, len(self.data)):
...
def caculate_tight_range(self, i, data):
    if i > 3:
        ...
        if len(self.list_tight_range) == 0:
            if std_deviation_list(list_zigzag_high, number) < 1 and std_deviation_list(list_zigzag_low, number) < 1:
                self.list_tight_range.append([list_points_high, list_points_low])
        
        elif len(self.list_tight_range) > 0:
            if self.list_tight_range[-1][0][-1][1] == point1_high and self.list_tight_range[-1][1][-1][1] == point1_low:
                
                self.list_tight_range[-1][0].append((i, point0_high))
                self.list_tight_range[-1][1].append((i, point0_low))
                
                if std_deviation_list([y[1] for y in self.list_tight_range[-1][0]], number) < 1 and std_deviation_list([y[1] for y in self.list_tight_range[-1][1]], number) < 1:
                    pass
                else:
                    self.list_tight_range[-1][0].pop()
                    self.list_tight_range[-1][1].pop()
            else:
                if std_deviation_list(list_zigzag_high, number) < 1 and std_deviation_list(list_zigzag_low, number) < 1 and i-11 > self.list_tight_range[-1][0][-1][0]:
                    self.list_tight_range.append([list_points_high, list_points_low])
    rt_tight_range = []
    for j in range(len(self.list_tight_range)):
        x1, x2= self.list_tight_range[j][0][0][0], self.list_tight_range[j][0][-1][0]
        y1 = max(self.list_tight_range[j][0], key=lambda x: x[1])[1]
        y2 = min(self.list_tight_range[j][1], key=lambda x: x[1])[1]
...
def check_partern_2hl_less(df, current_index, main_entry_direction, touch_keylevel_time):
    ...
    #         number_intersec = i-2
    #         break
    if str(current_index) == "2024-12-05 00:45:00" or str(current_index) == "2024-12-05 08:15:00" :
        print("xxxx")
    for i in range(len(x_zigzag_etf)):
        if int(datetime.strptime(list_intersec_etf[-1]["time"], "%Y-%m-%d %H:%M:%S").timestamp()) < int(datetime.strptime(x_zigzag_etf[i], "%Y-%m-%d %H:%M:%S").timestamp()):
            number_intersec = i
    ...
    if main_entry_direction == "BUY":
        ...
        if (index_L1 != None
                ...
                index_max_2 = y_zigzag_etf.index(max(y_zigzag_etf[index_L1:]))
                if (abs(min(y_zigzag_etf[index_L1 + 1:]) - y_zigzag_etf[index_L1]) < (max(y_zigzag_etf[index_L1:last_index_of(y_zigzag_etf, min(y_zigzag_etf[index_L1 + 1:]))]) - y_zigzag_etf[index_L1])/2
                    and last_index_of(y_zigzag_etf, min(y_zigzag_etf[index_L1 + 1:])) < len(y_zigzag_etf) - 1
                    and len(x_zigzag_etf) - last_index_of(y_zigzag_etf, min(y_zigzag_etf[index_L1 + 1:])) > 1

                    and time_to_candle(x_zigzag_etf[y_zigzag_etf.index(max(y_zigzag_etf[index_L1:last_index_of(y_zigzag_etf, min(y_zigzag_etf[index_L1 + 1:]))]))], x_zigzag_etf[last_index_of(y_zigzag_etf, min(y_zigzag_etf[index_L1 + 1:]))], time_frame="m15") > 3
                    and abs(y_zigzag_etf[y_zigzag_etf.index(max(y_zigzag_etf[index_L1:last_index_of(y_zigzag_etf, min(y_zigzag_etf[index_L1 + 1:]))]))] - y_zigzag_etf[last_index_of(y_zigzag_etf, min(y_zigzag_etf[index_L1 + 1:]))]) > 5
                    and time_to_candle(x_zigzag_etf[last_index_of(y_zigzag_etf, min(y_zigzag_etf[index_L1 + 1:])) ], x_zigzag_etf[-1], time_frame="m15") > 6
                    and abs(y_zigzag_etf[last_index_of(y_zigzag_etf, min(y_zigzag_etf[index_L1 + 1:]))] - y_zigzag_etf[y_zigzag_etf.index(max(y_zigzag_etf[last_index_of(y_zigzag_etf, min(y_zigzag_etf[index_L1 + 1:])) :]))]) > 5
                    ):
                    if index_max_2 > last_index_of(y_zigzag_etf, min(y_zigzag_etf[index_L1 + 1:])):
                        entry_price = [min(y_zigzag_etf[index_L1 + 1:]) - 1, min(y_zigzag_etf[index_L1 + 1:]) + 2]
                    else:
                        entry_price = [y_zigzag_etf[index_L1] - 1, y_zigzag_etf[index_L1] + 2]
    ...
    if main_entry_direction == "SELL":
        ...
        if (index_H1 != None
                ...
                if time_to_candle(x_zigzag_etf[index_H1], current_index.strftime('%Y-%m-%d %H:%M:%S'), "m15") > 30:
                    index_L1_2 = y_zigzag_etf.index(min(y_zigzag_etf[index_H1:]))
                    if (abs(max(y_zigzag_etf[index_H1 + 1:])- y_zigzag_etf[index_H1]) < (y_zigzag_etf[index_H1] - min(y_zigzag_etf[index_H1:last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))]))/2
                    and last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:])) < len(y_zigzag_etf) - 1
                    and len(x_zigzag_etf) - last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:])) > 1
                    # and time_to_candle(x_zz_htf[index_max], x_zz_htf[y_zz.index(max(y_zz[index_max + 1:]))], time_frame="m15") > 15
                    and time_to_candle(x_zigzag_etf[y_zigzag_etf.index(min(y_zigzag_etf[index_H1:last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))]))], x_zigzag_etf[last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))], time_frame="m15") > 3
                    and abs(y_zigzag_etf[y_zigzag_etf.index(min(y_zigzag_etf[index_H1:y_zigzag_etf.index(max(y_zigzag_etf[index_H1 + 1:]))]))] - y_zigzag_etf[last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))]) > 5
                    and time_to_candle(x_zigzag_etf[last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))], x_zigzag_etf[-1], time_frame="m15") > 6
                    and abs(y_zigzag_etf[last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:]))] - y_zigzag_etf[y_zigzag_etf.index(min(y_zigzag_etf[last_index_of(y_zigzag_etf, max(y_zigzag_etf[index_H1 + 1:])) :]))]) > 5
