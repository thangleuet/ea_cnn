    ...
    for i in tqdm(range(len(df_test))):
        ...
        if df_test['predicted_labels'].iloc[i] == 1 and df_test['confidence'].iloc[i] > 0.5:
            ...
            for j in range(i+1, len(df_test)):
                if not is_order and df_test["high"].iloc[j] >= key_level >= df_test["low"].iloc[j]:
                    window = df_test['low'].iloc[i+1:j]
                    # if len(window) > 10 and window.min() - key_level < 2:
                    #     break    
                    entry_price = key_level
                    entry_time = df_test['Date'].iloc[j]
                    is_order = True
                if is_order:
            ...
            for j in range(i+1, len(df_test)):
                if not is_order and df_test["high"].iloc[j] >= key_level >= df_test["low"].iloc[j]:
                    window = df_test['high'].iloc[i+1:j]
                    # if len(window) > 10 and key_level - window.max() < 2:
                    #     break
                    entry_price = key_level
                    entry_time = df_test['Date'].iloc[j]
                    is_order = True
