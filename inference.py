    ...
    for i in range(2, 21):
        if index + i < len(df_raw):  # Ensure we don't go out of bounds
            ...
            if predicted_class == 1:
                sl = max(df_raw.iloc[index+1]['low'] - min(df_raw.iloc[index]['low'], df_raw.iloc[index+1]['close']) + 1, 5)
                # Filter 
...
csv_path = "val/features_val.csv"
df_raw = pd.read_csv(csv_path)
df_raw['timestamp'] = pd.to_datetime(df_raw['timestamp'])

features = list(df_raw.columns[1:-1])
input_data = df_raw.loc[:, features].values

# Load scaler
scaler = np.load('weights/scaler.npy', allow_pickle='TRUE').item()
input_scale = scaler.transform(input_data)

# normalization_params = joblib.load('normalization_params.pkl')  # Hoặc sử dụng pickle
# log_min = normalization_params['log_min']
# log_max = normalization_params['log_max']
# input_data_log = np.log(input_data)
# input_scale = (input_data_log - log_min) / (log_max - log_min)
